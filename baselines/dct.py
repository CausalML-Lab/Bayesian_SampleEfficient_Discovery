...
def create_random_dag_from_undirected_graph(g:nx.Graph):
    DAG = nx.DiGraph()
    edges = []
...
def get_components(d:nx.DiGraph):
    UCCG = graphical_models.PDAG(nodes=d.nodes, edges=d.edges)
    d_gm = graphical_models.PDAG(nodes=d.nodes, arcs=d.edges)
    colliders = d_gm.vstructs()
    for a, b in list(colliders):
        if set( (a, b) ) in UCCG.edges:
            UCCG.replace_edge_with_arc( (a, b) )

    UCCG.to_complete_pdag()
    components = UCCG.chain_components()

    return components

def dag_from_component(c:graphical_models.PDAG):
    d = nx.DiGraph()
    n_ids = np.arange(len(c.nodes))
    node_id = {}
    i = 0
    ...
    for a, b in list(c.edges):
        edges.append( (node_id[a], node_id[b]) )
            ...
            for parent_clique in previous_layer_cliques:
                for d in range(degree):
                    if counter < nnodes:
                        ...
                        intersection_size = min(len(parent_clique) - 1,
                                                random.randint(int(clique_size / 2), clique_size - 1))
...
def random_chordal_graph2(nnodes: int, k: int, ngraphs: int = 1, ensure_connected=True) -> Union[
    List[nx.DiGraph], nx.DiGraph]:
    if ngraphs == 1:
        for ix in itr.count():
            if ix > 100:
...
def simulate_clique_intervention(dcg: LabelledMixedGraph, cpdag: PDAG, dag: DAG, intervened_clique: frozenset,
                                 simple=False, bn=None, t_samples=0, data=None):
    # print('hi')
    I_count = 0
    """
...
def apply_clique_intervention2(
        ...
        dag=None,
        check=True) -> (LabelledMixedGraph, LabelledMixedGraph, set):
    """
        ...
        if current_unoriented_edges == new_unoriented_edges:
            if not extra_interventions:
                ...
                for (c1, c2), label in current_unoriented_edges.items():
                    c3 = next(
                        (c3 for c3 in new_clique_graph.onto_nodes(c1) & new_clique_graph.onto_nodes(c2) if
                         (c3 & c1) == (c3 & c2))
                        , None
                    )

                    if c3 is not None and (
                            c3 & c1) < label:  # if c1 and c2 have a common parent/spouse with a smaller intersection, then spend interventions
                        if verbose: print(
                            f"Intervening on extra nodes {c1 & c2} to orient {c1}-{c2} with common_parent = {c3}")
...
def DCT_discovery(a, nodes, m):
    ...
    import numpy as np
    while (1):
        d = nx.barabasi_albert_graph(nodes, m)
        d = create_random_dag_from_undirected_graph(d)
        c = get_components(d)
        if len(c) != 1:
            continue
        a = dag_from_component(c[0])
    ...
    for j in range(1):
        ...
        for t in range(500, 100000, 500):
            ...
            while True:
                ...
                # INTERVENE ON THE CENTRAL CLIQUE
                central_clique = get_tree_centroid(current_clique_subtree.to_undirected(as_view=True),
                                                   verbose=False)
                if verbose:
                    print(f'Picked central clique: {central_clique}')
                I_count, cpdag, intervened_nodes_central = simulate_clique_intervention(dcg, cpdag, dag,
                                                                                        central_clique, bn=bn,
                                                                                        t_samples=int(
                                                                                            t_samples / 1),
                                                                                        data=data)
            ...
            for next_component in sorted_components:
                ...
                residual_nodes = frozenset.union(*next_component)
                if len(residual_nodes) > 1:
                    I_count, cpdag, intervened_nodes_inside = intervene_inside_clique(cpdag, dag, residual_nodes,
                                                                                      bn=bn,
                                                                                      t_samples=int(t_samples / 1),
                                                                                      data=data)
                    I_fcount = I_fcount + I_count
            ...
            shd[t_index:t_index + 500 * I_fcount] = len(cpdag.arcs - bn.arcs()) + len(bn.arcs() - cpdag.arcs)
            t_index = t_index + 500 * I_fcount
        ...
        print(shd[0:t_index - 500 * I_fcount])
        print('***********')
        Data_save.append(shd[0:t_index - 500 * I_fcount])
        samples_list.append(t_index - 500 * I_fcount)

    return Data_save, samples_list