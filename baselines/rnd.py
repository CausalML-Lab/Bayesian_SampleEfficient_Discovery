...
class MPDAG:
    ...
    def apply_meek_rules(self):
        ...
        for a, b, k, l in itertools.permutations(self.nodes, 4):
            if k != l and is_nonadjacent(k, l) and ((a, k) in self.Edges or (k, a) in self.Edges) and (
                    (a, l) in self.Edges or (l, a) in self.Edges) and (k, b) in self.Arcs and (l, b) in self.Arcs and (
                    (a, b) in self.Edges or (b, a) in self.Edges):
                self.addarc(a, b)
                break
        ...
        for a, k, l, b in itertools.permutations(self.nodes, 4):
            if k != b and is_nonadjacent(k, b) and (k, l) in self.Arcs and (l, b) in self.Arcs and (
                    (a, k) in self.Edges or (k, a) in self.Edges) and ((a, b) in self.Edges or (b, a) in self.Edges):
                self.addarc(a, b)
                break
...
def Random_Interventions(a, nodes, degree):
    import numpy as np
    import math
    ...
    #r = list(np.linspace(0, nodes-1, nodes))
    ...
    for j in range(1):
        ...
        for t in range(1000, 100000, 1000):
            ...
            shd[samples:samples + math.floor(avg_int_size * 1000)] = len(Truedag - arcs)
            samples = samples + math.floor(avg_int_size * 1000)

            # print(len(Truedag-arcs))
        #print(arcs)
        #print(Truedag)
        #print(len(Truedag - arcs))
        print(samples - math.floor(avg_int_size * 1000))
        print('*************************')
        shd[0:500] = len(bn.arcs())
        Data_save.append(shd[0:samples - math.floor(avg_int_size * 1000) + 1])
        samples_list.append(samples - math.floor(avg_int_size * 1000))
        print(samples_list)
